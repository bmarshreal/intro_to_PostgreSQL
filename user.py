...
class User:
    ...
    def save_to_db(self):  # Create our method to fetch data from DB
        with CursorFromConnectionFromPool() as cursor:  # Connect to DB from connection pool and close connect after method 'save_to_db' is finished
    ...
    def load_from_db_by_email(cls, email):  # Create our method to fetch data from DB
        with CursorFromConnectionFromPool() as cursor:  # Connect to DB from connection pool and close connection
            # Create a cursor to navigate through DB
            ...
            return cls(email=user_data[1], first_name=user_data[2], last_name=user_data[3],
                       id=user_data[0])  # Return data from DB
